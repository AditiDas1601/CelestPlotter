...
LONG_DESCRIPTION="Use this package to make converting Right Ascension and Declination values to degrees(from HMS and DMS) way easier. Simply use InNCon.Convert(...) to convert the parameters into degrees. Also use InNCon.In() for a text based interface to input your values. This package also has a plotting function CelSphere.Plot() to plot the RA and Declination values on the Celestial Sphere, complete with the representation of the Equatorial Plane and vector marking the Vernal Equinox."
# Setting up
setup(
        name="CelestPlotter", 
        version=VERSION,
